...
{
    /// <summary>
    /// Manages the TTL heartbeat that updates the Zookeeper discovery table row entry for this cluster node.
    /// Instantiated as a child of the ZookeeperDiscoveryGuardian actor, only after it initialized properly.
    /// Heartbeat is based on the akka.discovery.zookeeper.ttl-heartbeat-interval setting, which defaults to 1 minute.
    /// </summary>
    internal sealed class HeartbeatActor: UntypedActor, IWithTimers
    {
        public static Props Props(ZookeeperDiscoverySettings settings, ClusterMemberZookeeperClient client)
        ...
        public HeartbeatActor(ZookeeperDiscoverySettings settings, ClusterMemberZookeeperClient client)
        {
            _client = client;
        ...
        private async Task<Status> ExecuteUpdateOpWithRetry()
        {
            ...
            await _client.CheckLeaderAsync(cts.Token);
